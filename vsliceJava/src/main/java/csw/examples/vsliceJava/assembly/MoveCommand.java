...
public class MoveCommand extends AbstractActor {
  ...
  public MoveCommand(AssemblyContext ac, Setup s, ActorRef tromboneHCD, TromboneState startState, Optional<ActorRef> stateActor) {
    this.ac = ac;
    this.s = s;
    ...
    return receiveBuilder().
        matchEquals(TromboneAssembly.CommandStart.instance, t -> {
          if (cmd(startState).equals(cmdUninitialized) || (!move(startState).equals(moveIndexed) && !move(startState).equals(moveMoving))) {
            ...
            DoubleParameter stagePosition = jparameter(s, AssemblyContext.stagePositionKey);
