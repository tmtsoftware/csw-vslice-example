...
object ParamValidation {
  ...
  def initValidation(sc: Setup)(implicit ac: AssemblyContext): Validation = {
    ...
    if (sc.size == 0)
      ...
      if (missing.nonEmpty)
        Invalid(MissingKeyIssue(s"The 2 parameter init Setup requires keys: $configurationNameKey and $configurationVersionKey"))
  ...
  def moveValidation(sc: Setup)(implicit ac: AssemblyContext): Validation = {
    if (sc.prefix != ac.moveCK) {
      ...
      if (!sc.exists(ac.stagePositionKey)) {
        ...
        Invalid(WrongUnitsIssue(s"The move Setup parameter: ${ac.stagePositionKey} must have units of: ${ac.stagePositionUnits}"))
      } else Valid
    }
  }
  ...
  def positionValidation(sc: Setup)(implicit ac: AssemblyContext): Validation = {
    if (sc.prefix != ac.positionCK) {
      ...
      if (!sc.exists(ac.naRangeDistanceKey)) {
        ...
        Invalid(WrongParameterTypeIssue(s"The position Setup must have a DoubleParameter named: ${ac.naRangeDistanceKey}"))
      } else if (sc(ac.naRangeDistanceKey).units != ac.naRangeDistanceUnits) {
        Invalid(WrongUnitsIssue(s"The position Setup parameter: ${ac.naRangeDistanceKey} must have units of: ${ac.naRangeDistanceUnits}"))
      } else {
  ...
  def setElevationValidation(sc: Setup)(implicit ac: AssemblyContext): Validation = {
    if (sc.prefix != ac.setElevationCK) {
      ...
      Invalid(WrongParameterTypeIssue(s"The setElevation Setup must have a parameter: ${ac.naElevationKey} as a DoubleParameter"))
    } else if (sc(ac.naElevationKey).units != ac.naRangeDistanceUnits) {
      Invalid(WrongUnitsIssue(s"The move Setup parameter: ${ac.naElevationKey} must have units: ${ac.naElevationUnits}"))
    } else Valid
  }
  ...
  def setAngleValidation(sc: Setup)(implicit ac: AssemblyContext): Validation = {
    ...
    if (!sc.exists(ac.zenithAngleKey)) {
      ...
      Invalid(WrongUnitsIssue(s"The setAngle Setup parameter: ${ac.zenithAngleKey} must have units: ${ac.zenithAngleUnits}"))
    } else Valid
  }
